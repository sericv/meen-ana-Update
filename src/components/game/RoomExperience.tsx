...
import { GuessReveal, type GuessRevealOutcome } from "@/components/game/GuessReveal";
...
export function RoomExperience({ roomId }: Props) {
  ...
  // ── "لحظة الكشف" — own-card reveal cinematic, shown to both players
  // before the result screen once the match has ended.
  const { myCard: revealedMyCard } = useRevealedCards(roomId, uid ?? "");
  const [guessRevealDone, setGuessRevealDone] = useState(false);

  const myLastGuessText = useMemo(() => {
    if (!uid) return null;
    for (let i = messages.length - 1; i >= 0; i -= 1) {
      const m = messages[i];
      if (m.type === "guess" && m.senderUid === uid) return m.text;
    }
    return null;
  }, [messages, uid]);

  const guessRevealOutcome: GuessRevealOutcome = useMemo(() => {
    if (!ended || !winnerUid) return "neutral";
    if (match?.winReason === "guess") return iWon ? "correct" : "neutral";
    if (match?.winReason === "guess_limit") return iWon ? "neutral" : "wrong-guess";
    return "neutral";
  }, [ended, winnerUid, match?.winReason, iWon]);

  useEffect(() => {
    setGuessRevealDone(false);
  }, [match?.id]);

  // Fallback: if the revealed card never arrives (e.g. reveal-cards
  // endpoint failure), skip the cinematic and go straight to results.
  useEffect(() => {
    if (!ended || revealedMyCard || guessRevealDone) return;
    const t = window.setTimeout(() => setGuessRevealDone(true), 3000);
    return () => window.clearTimeout(t);
  }, [ended, revealedMyCard, guessRevealDone]);
  ...
  useEffect(() => {
    if (!ended || !iWon || !winnerUid || !guessRevealDone) return;
    if (!confettiDone.current) {
      confettiDone.current = true;
      playWinSparkle();
    }
  }, [ended, iWon, winnerUid, guessRevealDone]);
  ...
  const openGuessFlow = useCallback(() => {
    ...
    if (phase !== "question") {
      toastBanner("يمكن التخمين فقط في دور السؤال");
      return;
    }
  ...
  }, [myTurn, phase, myGuessRemaining, toastBanner]);
  ...
  return (
    ...
    >
      <ConfettiBurst active={ended && iWon && Boolean(winnerUid) && guessRevealDone} />
      ...
      {/* ════════════════════════════════════════════════════════
          GUESS REVEAL — «لحظة الكشف» cinematic, shown to both
          players for their own card before the result screen.
      ════════════════════════════════════════════════════════ */}
      {ended && !guessRevealDone && revealedMyCard ? (
        <GuessReveal
          won={iWon}
          outcome={guessRevealOutcome}
          guess={myLastGuessText ?? undefined}
          actual={revealedMyCard.nameAr || revealedMyCard.name}
          category={getCategoryById(revealedMyCard.categoryId)?.nameAr ?? revealedMyCard.categoryId}
          imageUrl={revealedMyCard.imageUrl}
          onDone={() => setGuessRevealDone(true)}
        />
      ) : null}
